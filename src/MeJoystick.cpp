...
#endif // ME_PORT_DEFINED
  x_value = (x_value - CENTER_VALUE_X) + _X_offset;
  if(x_value<0){
	  x_value = (x_value*512/735);
  }else{
	  x_value = (x_value*512/282);
  }
  return x_value;
}
...
#endif // ME_PORT_DEFINED
  y_value = (y_value - CENTER_VALUE_Y) + _Y_offset;
  if(y_value<0){
	  y_value = (y_value*512/758);
  }else{
	  y_value = (y_value*512/260);
  }
  return y_value;
}
